...
Generally, jobscripts will use something like the following structure (see our section on [Example Jobscripts](3.4-Example-Jobscripts.md) for more examples):
```bash
#!/bin/bash -l
...
If you want to avoid sparse process placement and your job taking up more nodes than you were expecting, the maximum memory request you can make when using all the cores in a standard node is 120/16 = 7.5G.
...
If you want to get more information on a particular job, note its job ID and then use the -f and -j flags to get full output about that job. Most of this information is circumstantial and may not be very useful most of the time, but can be very helpful in determining what's wrong if things are not working as expected.
```
qstat -f -j 12345
```
#### Job states

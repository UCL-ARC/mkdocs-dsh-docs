...
If you wish to request a specific type of GPU, you can use the corresponding boolean operator as noted in the table below, for example:
```
#$ -l gpu=1
#$ -l a100=TRUE
```
...
If you have created a custom virtual environment that you would like to use for your job (e.g. using `conda ` or `venv`), include this information in your job submission script so that the cluster knows which environment to use. For example, if using a Conda environment for your job, you would add the line `conda activate myenv` to your job script, where "myenv" is the name of your desired environment.

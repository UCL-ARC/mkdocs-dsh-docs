...
When working in the DSH HPC environment, you can install software available via Artifactory into your own space (either the DSH HPC Cluster, or your Customer Specialist Server). **You can only access the DSH Artifactory instance from inside the DSH.**
...
!!! Note
    While DSH Desktop uses a Windows environment, the DSH HPC environment uses Linux. The keyboard combination "Ctrl+V" will not work to paste things into Linux -- use "Shift+Insert" instead, which works in both Windows and Linux.

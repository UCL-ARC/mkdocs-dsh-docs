...
When working in the DSH HPC Services environment, you can install software available via Artifactory into your own space (either the DSH HPC Cluster, or your Customer Specialist Server). **You can only access the DSH Artifactory instance from inside the DSH.**
...
After you log in, you will be shown the Packages page by default, which allows you to browse the available packages. Note that due to the security-focused nature of the DSH, there will be some packages or versions that are unavailable. If the package that you are looking for is not present you may be able to [request it](3-DSH_Cluster.md/#requesting-software-installs). Note, however, that there may be some software that simply poses an unacceptable risk.
...
!!! Note
    While DSH Desktop uses a Windows environment, the DSH HPC Services environment uses Linux. The keyboard combination "Ctrl+V" will not work to paste things into Linux -- use "Shift+Insert" instead, which works in both Windows and Linux.

!!! important 
    The Artifactory token is tied to your current DSH account password. While they are less sensitive than your password, tokens should nonetheless be treated similarly to passwords in terms of keeping them secret.
...
Since our users do not have administrative privileges on DSH HPC machines, you cannot install anything requiring `sudo`. If the instructions tell you to do that, read further to see if they also have instructions for installing in user space, or for doing an install from source if they are RPMs.

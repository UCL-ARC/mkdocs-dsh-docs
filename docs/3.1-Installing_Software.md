...
!!! Note
Since most of these configuration files and directory names begin with a `.`, they will be treated as hidden in Linux by default. To see these items in your DSH HPC cluster home environment, you can use a command like `ls -ahlp ~` to list all files in your home directory, including hidden files.
...
!!! Note
This file location and name are different from the DSH Desktop environment. Make sure you've named the file correctly and placed it in the correct folder structure.

...
All connections are via SSH within the DSH portal, and you use your UCL credentials to log in. UCL services can only be connected to by users inside the UCL network which may mean using the institutional VPN or "bouncing" off another UCL machine when [accessing them from outside the UCL network](howto.md#logging-in-from-outside-the-ucl-firewall). 
### Login hosts

|Service | General alias          | Direct login node addresses                            |
|--------|------------------------|--------------------------------------------------------|
|DSH     | `DSH.rc.ucl.ac.uk`     | `login01.DSH.rc.ucl.ac.uk`  `login02.DSH.rc.ucl.ac.uk` |

Generally you should connect to the general alias as this is load-balanced across the available login nodes.
...
All UCL services use the same software stack based upon RHEL 7.x with a standardised set of packages available for the users. 

## Batch System

UCL services use Grid Engine to manage jobs. 
